...
| `dir` | 데이터 디스크(`C:`) 루트 디렉터리의 파일 목록을 표시합니다. |
| `task` | 실행 중인 태스크 목록을 Debug Window에 표시합니다. |
| `taskmgr` | 태스크 매니저 창을 엽니다. |
| `disk` | ATA 디스크 인식 정보와 데이터 디스크 부트 섹터를 표시합니다. |
| `touch <파일>` | 빈 파일을 데이터 디스크에 만듭니다. 이미 있으면 그대로 둡니다. |
| `rm <파일>` | 데이터 디스크에서 파일을 삭제하고 FAT 클러스터를 해제합니다. |
| `cp <원본> <대상>` | 파일을 raw bytes 그대로 복사합니다. 예: `cp a.he2 b.he2` |
| `mv <원본> <대상>` | 파일을 이동하거나 이름을 바꿉니다. 내부적으로 복사 후 원본을 삭제합니다. |
| `echo <문자열> > <파일>` | 문자열과 줄바꿈을 파일에 저장합니다. 기존 파일은 덮어씁니다. |
| `mkfile <파일> <바이트>` | A-Z 반복 패턴으로 지정한 크기의 파일을 만듭니다. 예: `mkfile big.bin 102400` |
