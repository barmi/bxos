 ...
 * 2003/09/27 | PICTURE0.BIN(DLL)용으로 개조 ( by 쿠민)
 * 2003/09/28 | 각종 버그 수정 ＆다소의 최적화 ( by H.Kawai )
 *
 */
...
typedef struct
{
    ...
    int scan_ac[3];
    int scan_dc[3];
    int scan_h[3];  // 샘플링 요소수
    int scan_v[3];  // 샘플링 요소수
	...
	int base_img[64][64]; // 기저 화상 ( [옆주파수 uπ][세로 주파수 vπ][옆위상(M/8)][세로 위상(N/8)]
...
//	if (jpeg->width == 0)
//		return 8;
	/* decode0에서는 info하고 나서 call되므로, 이것은 없다 */
...
unsigned short get_bits(JPEG *jpeg, int bit)
{
	...
	while (remain <= 16) {
		...
		c = *jpeg->fp++;
		if (c == 0xff) { // 에러를 막기 위해 FF -> FF 00에 escape되고 있다
			if (jpeg->fp >= jpeg->fp1) {
				ret = 0;
				goto fin;
			}
...
int jpeg_dht(JPEG *jpeg)
{
	...
	while (len > 0) {
		...
		tc = (val >> 4) & 0x0f; // 테이블 클래스(DC/AC성분 selector)
		th =  val       & 0x0f; // 테이블 헤더
...
void jpeg_idct(int *block, int *dest, int base_img[64][64])
{
    ...
    // 고정 소수점을 원래대로 되돌리기 위해 +4로 나눈다
    for (i = 0; i < 64; i++)
        dest[i] >>= 17;
    return;
}

// encode된 수치를 원래대로 되돌린다
...
int jpeg_decode_mcu(JPEG *jpeg, UCHAR *zigzag_table)
{
	...
	// mcu_width x mcu_height 사이즈의 블럭을 전개
