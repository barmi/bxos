...
- work6 은 **Phase 3 구현 완료 → QEMU 측정 대기 → Phase 4 진입 직전** 단계다.
...
6. **Phase 4 진입** — Dirty rect 인프라 (커널 측 SHEET 에 `dirty_rect[4]` 누적,
   `sheet_dirty_add` / `sheet_dirty_flush` / idle flush). 호환 mode 에서 시작
   (`sheet_refresh` 즉시 동작 유지). baseline ↔ Phase 2 ↔ Phase 3 의 측정값이
   work6-bench.md 에 채워져 있어야 Phase 4 의 인프라 단계가 회귀 없는지 검증 가능.
7. **시나리오 S1~S5 표** — 이후 phase 종료마다 같은 표를 다시 채워 비교한다.
