...
- work6 은 **Phase 0 잠금 직전 단계**다 (계획서만 작성, 구현 미시작).
- 작업트리는 work5 Phase 8 (`ea345d6` 시점) 클린 상태.
...
- **측정 단위**: PIT 10ms tick + call count + max. RDTSC 회피.
...
struct BENCH_COUNTER {
    ...
    int enabled;
    uint32_t calls;
    uint32_t total;     /* PIT count 누적 */
    uint32_t max;       /* PIT count 최대 */
};
...
2. **Phase 0 잠금** — work6.md §2 결정 표 검토 / 동의 후 commit.
