...
- work6 은 **Phase 4 구현 완료 → QEMU 회귀 smoke 대기 → Phase 5 진입 직전** 단계다.
...
- Phase 4 는 **인프라 단계** — sheet 단위 dirty rect 누적 + idle flush. `sheet_refresh`
  자체 동작은 변경 없어 회귀 가능성 작음. Phase 5 의 부분 redraw 가 본격 사용.
- 측정 인프라 (`bench.c/h`, hot path 9곳, `bench` 콘솔 명령) 가 들어간 상태.
- 첫 baseline 숫자는 사용자가 QEMU 안에서 직접 측정해 [_doc/work6-bench.md](work6-bench.md)
  의 표 (절차 / S1~S5 정의 / Phase 별 결과 슬롯) 에 채워야 한다 (이후 phase 비교 기준).
- 신규 syscall edx 번호는 44~47 로 잠금 (work6.md §8). work5 까지 1~43 사용 중.
- Phase 0 검증 결과는 work6.md §3 Phase 0 표가 정본.
- Phase 1 구현 변경 (work6.md §3 Phase 1 노트):
  - 신규 [harib27f/haribote/bench.c](../harib27f/haribote/bench.c) + [bench.h](../harib27f/haribote/bench.h).
  - 9개 hot path 에 `bench_enter`/`bench_leave` (sheet_refreshmap/refreshsub/sheet_slide/
    boxfill8/putfont8/putfonts8_asc/taskbar_full_redraw/scrollwin_redraw/hrb_api).
  - `hrb_api` 는 어셈 entry 유지 위해 wrapper + `static hrb_api_inner` 로 분리.
  - 콘솔 `bench` 명령 (on/off/reset/dump/mark) ([cmd_bench](../harib27f/haribote/console.c)).
  - 64-bit 나눗셈은 `bench_div_u64_u32` 로 직접 구현 — libgcc 비의존.
  - `CMakeLists.txt` `BXOS_KERNEL_C_NAMES` 에 `bench` 추가, `BXOS_KERNEL_HEADERS` 에 `bench.h`.
  - `bench_init()` 는 HariMain 부팅 시 `init_palette` 직전 호출.
...
7. **Phase 5 진입** — Taskbar / scrollwin / 마우스 부분 redraw. `taskbar_full_redraw` 를
   `taskbar_redraw_start` / `_clock` / `_winlist` 로 분리, scrollwin 의 append/scroll
   분리, 마우스 sheet_slide 같은 좌표 skip. dirty rect 위에서 동작. **이 phase 가
   measurable 한 큰 폭 향상** (특히 S2 dir × 10 의 scrollwin 30 G cycles 절감 후보).
8. **시나리오 S1~S5 표** — 이후 phase 종료마다 같은 표를 다시 채워 비교한다.
