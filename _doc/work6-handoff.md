...
- work6 은 **Phase 1 구현 완료 → QEMU baseline 측정 대기 → Phase 2 진입 직전** 단계다.
...
3. ~~Phase 1 구현~~ — 완료 (2026-05-01). bench 인프라 + hot path 9곳 + `bench` 콘솔
   명령 + work6-bench.md. 남은 일은 QEMU 안에서 baseline 측정 후 표 채움 (사용자
   직접 — `bench on/off` 토글 후 S1~S5 시나리오 실행하고 dump 결과 기록).
4. **Phase 2 진입** — Quick wins: `boxfill8` 을 row 당 `memset` 으로, `sheet_refreshsub`
   1바이트 path 에 run-length memcpy, 4바이트 path 정렬 보정, `putfonts8_asc_ascii`
   신규 entry. 각 변경 후 같은 시나리오로 재측정해 work6-bench.md Phase 2 표 채움.
5. **시나리오 S1~S5 표** — 이후 phase 종료마다 같은 표를 다시 채워 비교한다.
