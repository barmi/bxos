...
- work6 은 **Phase 2 구현 완료 → QEMU 측정 대기 → Phase 3 진입 직전** 단계다.
...
5. **Phase 3 진입** — Compositor 일반화 + 폰트 lookup. `sheet_refreshmap` 의 4-byte
   path 일반화, `hankaku_fast[256][16][8]` 32KB lookup table. baseline 과 Phase 2
   사이의 비교 표가 work6-bench.md 에 기록되어 있어야 Phase 3 효과 평가 가능.
6. **시나리오 S1~S5 표** — 이후 phase 종료마다 같은 표를 다시 채워 비교한다.
