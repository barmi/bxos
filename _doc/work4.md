...
### Phase 6 — 파일관리 동작 (2일) — 구현 완료 (2026-05-01, QEMU smoke 대기)
...
- ☑ `m` 또는 toolbar Rename: rename/move.
  - ☑ 새 이름 또는 경로 입력.
  - ☑ `api_rename` 호출. cross-parent move 는 기존 syscall 정책대로 실패 status 를 표시한다.
- ☑ 동작 후 tree/file list reload 및 selection 보정.
- ☑ root 의 `.`/`..` 또는 없는 선택에 대한 방어 처리.

**구현 노트 (2026-05-01)**
- [explorer.c](../harib27f/explorer/explorer.c) 에 status bar 기반 input/confirm mode 를 추가했다. `n` 은 새 디렉터리명, `m` 은 새 이름/경로, `d` 는 `y/n` confirm 을 받는다.
- toolbar 의 `R/N/D/M` 버튼을 keyboard 동작과 같은 경로에 연결했다. `<-` 와 `..` 는 parent 이동으로 연결했다.
- 입력값은 FAT 8.3 component 기준으로 검증하고 소문자는 대문자로 정규화한다. `.`/`..`, 빈 이름, 너무 긴 이름, 잘못된 문자는 거부한다.
- 작업 성공 후 current directory tree 를 재구성하고 file list selection 을 생성/rename 된 항목 또는 삭제 지점 근처로 보정한다.
...
### Phase 7 — UI polish / 오류 처리 / 성능 정리 (1일) — 구현 완료 (2026-05-01, QEMU smoke 대기)
- ☑ 긴 목록 스크롤 polish:
  - ☑ selection 이 화면 밖으로 나가면 list/tree offset 갱신.
  - ☑ mouse wheel 대체로 tree/list scrollbar 를 제공한다.
  - ☑ PageUp/PageDown 또는 scrollbar arrow 버튼 추가 여부 검토 → app key ABI 에 PageUp/PageDown 이 없으므로 scrollbar top/bottom arrow 영역을 추가했다.
...
**구현 노트 (2026-05-01)**
- scrollbar 에 top/bottom arrow hit zone 을 추가했다. arrow click 은 1 row 씩, track click 은 기존처럼 1 page 씩 이동한다.
- toolbar button 은 fixed geometry 를 유지하며 hover/pressed/disabled 표현을 구분한다. 입력/confirm 중에는 button 을 disabled 처리한다.
- 성공/정보성 status 는 다음 list 입력에서 clear 하고, error/status prompt 는 유지한다.
- file list 색상은 directory=dark blue, `.HE2` executable=dark green, 일반 파일=black, selection 은 focus 여부에 따라 blue/gray 로 통일했다.
- 파일관리/open 경로에서 path overflow 를 감지해 `path too long` status 를 표시한다.
...
### Phase 8 — 문서 / 회귀 검증 / 마무리 (1일) — 구현 완료 (2026-05-01, QEMU smoke 대기)
- ☑ [BXOS-COMMANDS.md](../BXOS-COMMANDS.md) 에 `explorer` 사용법 추가 — 앱 표에 `explorer`/`lsdir`/`evtest` 항목과 키보드/마우스/파일 동작/cwd 정책을 정리한 “explorer 사용법” 섹션 추가.
- ☑ [README.utf8.md](../README.utf8.md) 빠른 시작 앱 목록 갱신 — HE2 25개 → 28개, `explorer` 한 줄 안내 추가.
- ☑ [SETUP-MAC.md](../SETUP-MAC.md) 에 explorer 빌드/실행/검증 단락 추가 (§2.6) — `install-explorer`, `bxos_fat.py ls`, `fsck_msdos`, 부팅 후 `explorer` 실행 흐름.
- ☑ [he2/README.md](../he2/README.md) / [he2/docs/HE2-FORMAT.md](../he2/docs/HE2-FORMAT.md) 에 새 syscall/event API 표 추가 — edx 1~43 디스패치 표와 `BX_DIRINFO`/`BX_EVENT` 구조체 노출.
