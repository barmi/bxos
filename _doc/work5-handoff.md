...
- work5 는 **Phase 1 구현 완료 → QEMU visual smoke 대기 → Phase 2 진입 직전** 단계다.
- Phase 1 코드 변경:
...
- 미존재:
  - Start Menu sheet, 실제 system tray 시계, taskbar window list, Alt+Tab.
...
4. **Phase 2 진입** — `menu.c` 를 먼저 임시 hard-coded items 로 띄워 동작 확인 → Phase 3 의 MENU.CFG loader 로 교체.
