...
- work5 는 **Phase 8 문서/마무리 완료 → QEMU full smoke 대기 → 종료 직전** 단계다.
- Phase 1 코드 변경:
  - `graphic.c`: `g_background_color`, `g_start_menu_open`,
    `taskbar_redraw(vram, x, y, start_hover, start_pressed)` 추가.
    `init_screen8()` 는 background fill + taskbar redraw 로 분리.
  - `bootpack.h`: taskbar hit-zone 상수와 신규 global/prototype 선언 추가.
  - `bootpack.c`: Start hover/press/release hit-test, Ctrl+Esc toggle 합성,
    `key_ctrl`/`key_alt` 추적 추가.
- Phase 1 호스트 검증 완료:
  - `cmake -S . -B build/cmake`
  - `cmake --build build/cmake`
  - `fsck_msdos -n build/cmake/haribote.img`
  - `fsck_msdos -n build/cmake/data.img`
- 남은 Phase 1 확인은 QEMU visual smoke 뿐이다:
  - Start 버튼 hover/press 가 보이는지.
  - Ctrl+Esc 로 Start 버튼 토글 상태가 보이는지.
  - 콘솔/explorer/tetris 마우스 회귀가 없는지.
- Phase 2 코드 변경:
  - `menu.c`: Start Menu primitive 추가. hard-coded root + `Programs` submenu 를
    `struct KERNEL_MENU` / `struct MENU_ITEM` 로 구성한다.
  - `bootpack.h`: `SHEET_FLAG_SYSTEM_WIDGET`, menu item/menu structs,
    `start_menu_*` API 선언 추가.
  - `bootpack.c`: Start button/Ctrl+Esc 가 `start_menu_toggle()` 을 호출하고,
    메뉴가 열려 있을 때 keyboard/mouse 를 menu layer 가 먼저 처리한다.
  - `CMakeLists.txt`: 커널 C source list 에 `menu` 추가.
- Phase 2 호스트 검증 완료:
  - `cmake -S . -B build/cmake`
  - `cmake --build build/cmake`
  - `fsck_msdos -n build/cmake/haribote.img`
  - `fsck_msdos -n build/cmake/data.img`
- 남은 Phase 2 확인은 QEMU visual smoke 뿐이다:
  - Start click/Ctrl+Esc 로 menu sheet 가 cursor 바로 아래 z-order 에 뜨는지.
  - `Programs` submenu 가 hover/Right 로 열리는지.
  - Up/Down/Left/Right/Enter/Esc, click-outside-close 가 정상인지.
- Phase 3 코드 변경:
  - `_doc/system/menu.cfg.default`, `_doc/system/settings.cfg.default` 추가.
  - `CMakeLists.txt`: `data.img` 생성 직후 `bxos_fat.py mkdir/cp` 로
    `/SYSTEM/MENU.CFG`, `/SYSTEM/SETTINGS.CFG` 설치.
  - `bootpack.h`: `KMENU_MAX_MENUS`, `KMENU_SECTION_MAX` 추가.
  - `menu.c`: `/SYSTEM/MENU.CFG` 를 `fs_data_open_path()`/`fs_file_read()` 로 읽는
    INI-like parser 추가. section `[start]`, `[start/Programs]`, `[item:<label>]`,
    `items = ...`, `handler = exec:/... | builtin:... | settings:... | submenu:...`
    를 static menu tree 로 변환한다.
  - config 누락/invalid root 시 Phase 2 hard-coded fallback 으로 부팅한다.
- Phase 3 호스트 검증 완료:
  - `cmake -S . -B build/cmake`
  - `cmake --build build/cmake`
  - `fsck_msdos -n build/cmake/haribote.img`
  ...
  - `MENU.CFG` 를 image 에서 HOST 로 copy 후 default 파일과 diff 일치 확인.
- 남은 Phase 3 확인은 QEMU config smoke 뿐이다:
  - `MENU.CFG` 항목을 바꾸고 재부팅하면 Start 메뉴가 따라 바뀌는지.
  - 잘못된 item/handler 한 줄만 skip 되고 나머지 메뉴는 유지되는지.
- Phase 6 코드 변경:
  - `bootpack.h`: `TASKBAR_WIN_X0/_BTN_MAX/_BTN_MIN_W/_BTN_MAX_W/_BTN_GAP/_TRAY_GAP`
    상수, `g_sht_back`/`g_buf_back` 선언, `taskbar_full_redraw`/`taskbar_mark_dirty`/
    `alt_tab_cycle`/`taskbar_winlist_hit`/`taskbar_winlist_sheet_at` API.
  - `bootpack.c`: 윈도우 목록 layout/draw/hit-test/Alt+Tab helper 추가,
    `taskbar_full_redraw` 가 background+Start+tray+winlist 를 한 번에 그림,
    `taskbar_mark_dirty` 는 dirty flag 로 다음 idle 직전에 redraw 트리거.
    HariMain 의 모든 `taskbar_redraw + sheet_refresh` 호출을 `taskbar_full_redraw`
    로 교체. mouse hover/press/release 가 윈도우 목록 버튼을 처리하고,
    keyboard 분기에 `Alt+Tab`/`Alt+Shift+Tab` 가 추가되었으며, 기존
    `Tab → window-cycle` 코드 (이전 line 746) 는 삭제.
    `keytable0[0x0f] = keytable1[0x0f] = 0x09` 로 단독 Tab 이 focused app 에
    char 0x09 로 전달되게 함.
- Phase 6 호스트 검증 완료:
  - `cmake -S . -B build/cmake`
  - `cmake --build build/cmake`
  - `fsck_msdos -n build/cmake/haribote.img`
  - `fsck_msdos -n build/cmake/data.img`
- 남은 Phase 6 확인은 QEMU smoke:
  - 부팅 직후 데스크톱이 비어 있고 (auto-console / auto-debug 제거) Start 버튼만 보임.
  - Start → Programs → Console 로 콘솔 띄움.
  - Start → Programs → Debug 로 debug 창 띄움 — taskbar 버튼에도 등장.
  - explorer + console + tetris 동시 실행 시 taskbar 가운데에 3개 버튼.
  - taskbar 버튼 click → 해당 창이 top + focus 로 sunken 표시.
  - Alt+Tab / Alt+Shift+Tab 순환.
  - 단독 Tab → explorer tree↔list focus 정상.
  - tetris/lines 입력 회귀 없음.
  - 모든 창을 닫고 나서도 시스템이 다운되지 않음 (key_win=0 fallback).
- Phase 7 코드 변경:
  - `bootpack.h`: `MENU_ITEM` 에 `char hotkey` 추가, `start_menu_handle_char` 선언.
  - `menu.c`: `hotkey =` config 키 파싱, hotkey underline 그리기 (`menu_hotkey_pos`),
    라벨 폭 초과 시 자르기 (`menu_label_fit`), `start_menu_handle_char()`로 hotkey
    매칭 & invoke. `menu_parse_handler` 실패 / 섹션 / item def 한계 시 dbg_putstr0.
    fallback 메뉴와 `menu.cfg.default` 모두 hotkey 부여.
  - `bootpack.c`: `SYSTEM_DIALOG` 에 `focus` 필드 추가 (0=input, 1=OK, 2=Cancel).
    `system_run_redraw` 가 focus 위치에 1px 점선 `system_draw_focus_ring`.
    `system_modal_handle_key` 가 Tab/Enter/Space 를 focus 위치 기반으로 처리.
    HariMain 키 dispatch 가 메뉴 열린 동안 char 도 consume → underlying app leak 차단.
    `settings_apply_pair` 에 `settings_warn` 으로 알 수 없는 key/value 를 debug
    window 에 누적.
- Phase 7 호스트 검증 완료:
  - `cmake -S . -B build/cmake`
  - `cmake --build build/cmake`
  - `fsck_msdos -n build/cmake/haribote.img`
  - `fsck_msdos -n build/cmake/data.img`
- 남은 Phase 7 확인은 QEMU smoke:
  - Start 메뉴를 열고 첫 글자 (E/C/T/D 등) 만 눌러도 항목이 실행되는지.
  - Programs 의 Tetris(`T`)와 Task Manager(`T`) 가 같은 hotkey 일 때 첫 매칭 항목 실행.
  - 메뉴 열린 동안 임의 키 (예: `q`) 를 눌러도 underlying console/explorer 로 문자 leak 없음.
  - Run 다이얼로그 Tab 으로 focus 순환, 점선 ring 표시, focus 위치 따라 Enter 동작.
  - MENU.CFG 의 한 줄을 `handler = nope:foo` 로 망가뜨려 부팅 → debug window 에
    `[menu] bad handler: …` 출력 + 그 항목만 빠지고 나머지 메뉴 정상.
  - SETTINGS.CFG 에 알 수 없는 key 를 추가해 부팅 → debug window 에 경고.
  - 메뉴 라벨이 21글자 넘는 경우 잘리고 끝이 `.` 로 표시.
...
- 부팅 후 화면 하단 28px 자리에 win95 톤 회색 taskbar 가 `init_screen8()`/`taskbar_redraw()`
  로 그려진다. 좌측 60×20 Start 버튼은 hover/press 상태가 있고, 우측 44×20 tray 에는
  `00:00` placeholder 가 보인다. 가운데 영역은 Phase 6 task list 를 위해 비워 둔다.
- 기존 GUI 인프라:
...
- 미존재:
  - 실제 handler dispatch, 실제 system tray 시계, taskbar window list, Alt+Tab.
...
| 1. Taskbar / Start 버튼 / tray ☑ | 2d | sht_back hit-test, hover/press, tray placeholder, Ctrl+Esc 합성 (host 검증 완료, QEMU visual smoke 대기) |
| 2. Menu sheet primitive ☑ | 1.5d | menu.c, cascading sub-menu, click-outside-close, keyboard nav (host 검증 완료, QEMU visual smoke 대기) |
| 3. MENU.CFG loader ☑ | 1.5d | `/SYSTEM/` 디렉터리, INI-like parser, 빌트인 default fallback (host 검증 완료, QEMU config smoke 대기) |
| 4. 핸들러 / 시계 / Run / About | 1.5d | exec/builtin dispatcher, 60s timer 시계, Run/About modal |
| 5. Settings 앱 ☑ | 2d | settings.he2, 스키마 자동 위젯, SETTINGS.CFG 영구 저장 (host 검증 완료, QEMU smoke 대기) |
| 6. Taskbar 윈도우 목록 / Alt+Tab ☑ | 1d | 가운데 영역 버튼, focus 토글, Alt+Tab 순환, Tab 회수 (host 검증 완료, QEMU smoke 대기) |
| 7. UI polish / 오류 처리 ☑ | 1d | hotkey underline, menu key leak fix, Run dialog Tab focus, parser 에러 debug 출력, 라벨 자르기 (host 검증 완료, QEMU smoke 대기) |
| 8. 문서 / 회귀 검증 ☑ | 1d | BXOS-COMMANDS / README / SETUP / he2/README 갱신, 신규 `_doc/menu-config.md`, work5 체크박스 갱신 (host 검증 완료, QEMU full smoke 대기) |
...
fsck_msdos -n build/cmake/data.img
python3 tools/modern/bxos_fat.py ls build/cmake/data.img:/SYSTEM
python3 tools/modern/bxos_fat.py cp build/cmake/data.img:/SYSTEM/MENU.CFG HOST:/tmp/bxos-menu.cfg
```
...
3. ~~Phase 1 진입~~ — 완료 (2026-05-01). `taskbar_redraw`, taskbar hit-test,
   `key_ctrl`/`key_alt`, Ctrl+Esc 합성, `g_background_color`, `g_start_menu_open`
   모두 반영. QEMU visual smoke 는 아직 남아 있다.
4. ~~Phase 2 진입~~ — 완료 (2026-05-01). `menu.c` primitive, hard-coded menu tree,
   keyboard/mouse routing, click-outside close, cursor 아래 z-order 반영.
   QEMU visual smoke 는 아직 남아 있다.
5. ~~Phase 3 진입~~ — 완료 (2026-05-01). `/SYSTEM` install, default cfg,
   INI-like loader, fallback menu, host 검증 반영. QEMU config smoke 는 아직 남아 있다.
...
8. ~~Phase 6 진입~~ — 구현 완료 (2026-05-01). taskbar 가운데에 윈도우 목록 버튼,
   focus 중인 창은 sunken, click 으로 focus 이동, Alt+Tab/Alt+Shift+Tab 순환,
   기존 Tab→z-order swap 제거 후 Tab(0x09) char 를 focused app 에 그대로 전달
   (explorer tree↔list focus 정상화). `taskbar_full_redraw` / `taskbar_mark_dirty`
   helper 가 Start/tray/winlist 를 한 번에 다시 그린다. QEMU smoke 는 아직 남아 있다.
9. Settings 카테고리/키 추가 시 work5.md §3 Phase 5 의 spec 표 한 줄과
   `harib27f/settings/settings.c` 의 `g_settings[]`/`g_categories[]` 를 같이 갱신.
10. ~~Phase 7 polish~~ — 구현 완료 (2026-05-01). hotkey underline (`hotkey = X`),
    menu key leak 차단 (열린 동안 모든 char consume), Run dialog Tab focus
    (input ↔ OK ↔ Cancel + 점선 ring), MENU.CFG / SETTINGS.CFG 파싱 에러를
    debug window 에 누적, 라벨 폭 초과 시 `.` 로 자르기. 보류한 한 가지: menu
    drop shadow (sheet 확장 비용 대비 효과 작음). QEMU smoke 는 아직 남아 있다.
11. ~~Phase 8 진입~~ — 문서 단계 완료 (2026-05-01). [BXOS-COMMANDS.md](../BXOS-COMMANDS.md)
    에 "Start Menu / Run / Settings" 단락, [README.utf8.md](../README.utf8.md)
    에 "시작 버튼 / 설정" 단락, [SETUP-MAC.md](../SETUP-MAC.md) 2.5 에
    `/SYSTEM/` 디렉터리 검증 명령, [he2/README.md](../he2/README.md) 에
    work5 신규 syscall 0개 문구, 신규 [_doc/menu-config.md](menu-config.md)
    스펙(파싱 규칙 + 핸들러 URI + 한계 + 호스트 검증 + fallback 정책) 추가.
    남은 일은 QEMU full smoke 뿐 — work5.md §3 Phase 8 의 체크리스트가 정본.
