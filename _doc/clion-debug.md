...
```bash
# 1) i386-elf-gdb 설치 — 32-bit Intel 타깃 (i386/i686 모두 같은 family).
#    brew core 에 'i686-elf-gdb' 는 없고 'i386-elf-gdb' 로 들어 있습니다.
#    바이너리 호환에는 차이가 없어 우리 i686-elf-gcc 빌드 산출물도 그대로 동작합니다.
...
| **GDB binary** | Settings → Build/Execution → Toolchains → Debugger 에서 `/opt/homebrew/bin/i386-elf-gdb` 지정 |
...
2. 새 toolchain 또는 기존 toolchain 의 **Debugger** 를 `Custom GDB` 로 바꾸고
   `/opt/homebrew/bin/i386-elf-gdb` 지정.
...
3. CLion 이 `i386-elf-gdb` 를 띄워 `tcp:1234` 로 attach. `bootpack.elf` 의 심볼이 자동 로드.
...
| LLDB 가 잡힘 (i386 DWARF 못 읽음) | CLion → Settings → Toolchains → Debugger 를 **Custom GDB** = `i386-elf-gdb` 로 변경. |
