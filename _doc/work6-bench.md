...
### 표 — Phase 7 (앱 측 polish) 후 — 측정 대기 (2026-05-04 구현 완료)
...
>   `draw_status`. `G_dirty` 비트마스크로 영역별 부분 redraw. (현 측정 시나리오에는
>   settings 가 없어 직접 영향 없음 — 사용자 체감 응답성 개선.)
> - **explorer.c**: `on_mouse_move` 가 visual 변화 (toolbar_hover/drag) 시만 redraw_all.
>   mouse 가 윈도우 위로 지나가는 경우 syscall 거의 0. (S3 의 explorer launch 측정에는
>   mouse_move 이벤트가 적어서 직접 영향 없음.)
> - **tetris.c / lines.c / evtest.c**: 변경 없음 (이미 효율적이거나 의도적 즉시 그리기).
>
> **예상 측정 결과**:
> - **S4 (tetris_t)**: 큰 폭 개선 예상.
>   - Phase 6: hrb_api 7,657 calls / 10.5 M cycles, refreshsub 51 / 5.5 M, boxfill8 5,482 / 3.2 M, **Σ 91.7 M**
>   - Phase 7 예상: hrb_api ≪ 1,000 calls (boxfilwin × 200 × redraw_count 사라짐), refreshsub 적음 (1회 sheet_refresh × redraw_count), boxfill8 거의 0 (cell 그리기는 user buffer 에서 일어남)
>   - 예상 Σ: ≤ 30 M (~3x 빠름)
> - **S1/S2/S3/S5**: 회귀 0건 (변경 영역 밖).
>
> **측정 절차**: P5/P6 와 동일.
> ```bash
> cmake --build cmake-build-release
...
>     -fda cmake-build-release/haribote.img \
>     -hda cmake-build-release/data.img -boot a
> # QEMU: > bench scenario → shutdown
> python3 tools/modern/bxos_fat.py cp \
>     cmake-build-release/data.img:/SYSTEM/BENCH.LOG /tmp/p7.log
> ```
