...
### work6 종료 정리 (2026-05-04)

Phase 1~7 측정 후 **효과 없이 오버헤드만 있는 부분** 식별 → 정리:

#### 되돌린 항목

1. **HariMain idle 의 `sheet_dirty_flush_all` 호출 제거**.
   - 진단: dirty rect 인프라 (`sheet_dirty_add` / `_flush`) 의 유일한 호출처가
     커널 syscall 핸들러 (`api_invalidate_rect`/`api_dirty_flush`) 인데, 그
     syscall 들은 실제 어떤 앱도 호출 안 함 (tetris_t 의 mode 1 분기는 default
     아님). 결과 idle path 의 256-sheet 순회는 매번 work 0.
   - P3→P4 측정에서 보였던 5~17% 회귀의 원인 중 하나로 추정.
2. **`taskbar_redraw_start_only` 함수 제거** — 호출처 0, compat helper 표기였음.

#### tetris.c (실제 게임) 에 measurement 결과 반영

- Line-clear 후의 `redraw_field` 가 cell × 200 `api_boxfilwin` 호출 → board_buf
  (28 KB) 에 직접 채우고 `api_blit_rect` 1회 (deferred). tetris_t 측정에서 검증된
  방식.
- Piece 움직임 (frame 당 4 cell erase + 4 cell draw) 은 그대로 boxfilwin —
  작은 rect 8개 매 frame 28 KB memcpy 는 오히려 회귀.
- score / level / lines 텍스트는 `draw_panel` 그대로 (변할 때만 redraw, 이미 효율적).

#### 유지하되 현재 사용처 0 (ABI / 미래 polish)

- `api_text_run` (edx 45) — 어떤 앱도 사용 안 함. work7 에서 explorer 라벨 / settings 라벨 도입 검토.
- `api_invalidate_rect` / `api_dirty_flush` (edx 46/47) — tetris_t mode 1 만, default 2 라 실제 0. disjoint 영역 다수 redraw 워크로드에 적합 — work7 의 explorer row-by-row 업데이트가 실제 사용처가 될 수 있음.
- `sheet_dirty_*` 인프라 + SHEET 의 dirty_rect[4][4] (8 KB) — 위 syscall 의 커널 구현. ABI 일관성 유지.

이 정리 후 `bench scenario` 재측정 시 idle path 호출 제거 효과만큼 미세한 차이 (S1/S5 같은 짧은 시나리오에서) 가능.

### 표 — Phase 7 (앱 측 polish) 후 (2026-05-04, `bench scenario` 자동 측정)

> Phase 7 변경 (앱 측):
> - **tetris_t.c**: `BENCH_MODE = 2` (default) 로 `api_blit_rect` 1회 경로 사용.
>   28KB BSS board_buf 에 cell 색 직접 채우고 winbuf 로 row-wise memcpy 한 번.
>   Phase 6 의 cell × 200 invalidate_rect syscall overhead 제거 → 401 → 1 syscall.
> - **settings.c**: `draw_settings()` 분리 → `draw_sidebar` + `draw_panel` +
>   `draw_status`. `G_dirty` 비트마스크로 영역별 부분 redraw.
> - **explorer.c**: `on_mouse_move` 가 visual 변화 (toolbar_hover/drag) 시만 redraw_all.
> - **tetris.c / lines.c / evtest.c**: 변경 없음.

#### Phase 7 — 모든 카운터 한눈에 보기 (auto)

| counter | S1 menu×10 | S2 dir×10 | S3 explorer | S4 tetris-line | S5 mouse |
|---|---:|---:|---:|---:|---:|
| `refreshmap` calls / total | 10 / 1.0 M | 0 / 0 | 1 / 0.3 M | 2 / 0.5 M | 2,000 / 5.9 M |
| `refreshsub` calls / total | 10 / 2.7 M | 9,903 / 279.7 M | 2 / 1.3 M | **18 / 25.8 M** | 2,000 / 7.8 M |
| `sheet_slide` calls / total | 15 / 0.004 M | 0 / 0 | 1 / 0 | 1 / 0.001 M | 1,001 / 14.2 M |
| `boxfill8` calls / total | 125 / 1.0 M | 0 / 0 | 108 / 0.9 M | **42 / 0.5 M** | 0 / 0 |
| `putfont8` calls / total | 6,825 / 1.4 M | 426,159 / 76.8 M | 11,821 / 3.7 M | 11,750 / 2.4 M | 8,039 / 1.6 M |
| `putfonts8_asc` calls / total | 0 / 0 | 0 / 0 | 39 / 0.07 M | 2 / 0.01 M | 0 / 0 |
| `taskbar` calls / total | 0 / 0 | 0 / 0 | 0 / 0 | 1 / 0.5 M | 0 / 0 |
| `scrollwin` calls / total | 517 / 31.0 M | 20,258 / 1,468.5 M | 503 / 62.5 M | 533 / 47.7 M | 495 / 31.4 M |
| `hrb_api` calls / total | 0 / 0 | 0 / 0 | 235 / 3.4 M | **17 / 7.3 M** | 0 / 0 |
| **Σ total cycles** | **37.1 M** | **1,825.0 M** | **72.2 M** | **84.8 M** | **60.9 M** |

#### Phase 7 시나리오 별 marks

| | s_N-start | s_N-end | elapsed | dump pit_tick |
|---|---:|---:|---:|---:|
| S1 | 1,764 | 1,765 | **1** (~10 ms) | 1,765 |
| S2 | 1,769 | 1,914 | **145** (~1.45 s) | 1,914 |
| S3 | 1,917 | 1,978 | **61** (~610 ms) | 1,979 |
| S4 | 1,985 | 2,135 | **150** (~1.5 s) | 2,136 |
| S5 | 2,140 | 2,142 | **2** (~20 ms) | 2,143 |

#### Phase 5 → 6 → 7 전체 비교 (Σ total cycles)

| 시나리오 | P5 | P6 | **P7** | P6→P7 | 비고 |
|---|---:|---:|---:|---:|---|
| S1 menu×10 | 53.1 M | 53.9 M | **37.1 M** | -31.2% | 10 ms 시나리오 — noise 영향 큼 (Phase 7 menu 경로 변경 없음) |
| S2 dir×10 | 1,778.7 M | 1,784.5 M | **1,825.0 M** | +2.3% | noise (콘솔 경로 변경 없음) |
| S3 explorer | 57.8 M | 76.2 M | **72.2 M** | -5.2% | noise (explorer launch 600 ms 측정만, mouse_move skip 효과 안 잡힘) |
| **S4 tetris-line** | **82.9 M** | **91.7 M** | **84.8 M** | **-7.5%** | **Phase 7 핵심** — 분석 ↓ |
| S5 mouse | 57.1 M | 55.7 M | **60.9 M** | +9.4% | noise (sheet_slide 직접 호출, 변경 없음) |

#### Phase 7 핵심 — S4 의 의외의 -7.5% (예상 -50%)

| counter | P6 calls / total | **P7 calls / total** | 변화 |
|---|---:|---:|---|
| `hrb_api` | 7,657 / 10.5 M | **17 / 7.3 M** | **calls -99.8% ✓** (cell × 200 boxfilwin + invalidate_rect 사라짐) |
| `boxfill8` | 5,482 / 3.2 M | **42 / 0.5 M** | **calls -99.2% ✓** (cell 그리기는 user buffer 에서) |
| `refreshsub` | 51 / 5.5 M | **18 / 25.8 M** (max=22.2 M) | **total +369% ⚠️** |
| `putfont8` | 11,639 / 3.8 M | 11,750 / 2.4 M | -36% (text run 안 변경, noise) |
| `scrollwin` | 530 / 67.6 M | 533 / 47.7 M | -29% (timing/cache noise — 콘솔 변경 없음) |

**관찰**:

**1. syscall 빈도는 의도대로 99% 절감 ✓** — 401 calls (200 boxfilwin + 200 invalidate + 1 flush) → 1 call (blit_rect).

**2. 그러나 `refreshsub` total 이 +20.3 M 회귀**.
- P6: dirty rect 4-cap union → 보드를 4 사각형으로 쪼개 refreshsub 호출 → 작은 영역 × 51 calls = 5.5 M.
- P7: `api_blit_rect` 1회가 보드 전체 (120×240=28,800 px) refreshsub 1회 호출 → **큰 영역 × 18 calls = 25.8 M**.
- **per-pixel cost 는 P7 이 ~3.8x 더 효율적** (49.7 cycles/px vs P6 의 191 cycles/px — 큰 영역의 cache-friendly memcpy 가 작은 영역의 setup overhead 절감).
- **그러나 absolute total 이 늘어남** — 큰 sheet_refreshsub 1회의 비용이 작은 4회의 합보다 큼.

**3. P7 S4 의 새 dominant 비용**:
- `scrollwin`: 47.7 M (**56%**) — `bench mark` / `bench dump` 가 콘솔에 putchar 출력하면서 발생. tetris_t 와 직접 무관.
- `refreshsub`: 25.8 M (**30%**) — blit_rect 의 큰 영역 refresh.
- `hrb_api`: 7.3 M (9%) — 거의 모두 사라짐.

**4. 결론 — `api_blit_rect` 의 진짜 win 조건**:
- syscall overhead 가 dominant 인 워크로드 ✓ (S4 P6 의 hrb_api 10.5M 에서 -3.2M 절감)
- refresh 면적이 syscall 횟수와 무관하게 작은 워크로드 (e.g. cell 단위 변경 1~2개) — 이 케이스에는 P7 이 가장 빛남.
- 보드 전체 redraw 같은 워크로드 (tetris_t) 는 P7 이 P6 보다 약간 빠름 (-7.5%) 정도.

**5. S1/S3 의 음(-)변화는 측정 noise**.
- S1 (10 ms 시나리오) 은 cache/timing 변동이 큼. Phase 7 의 explorer/settings/tetris_t 변경은 S1 (Start 메뉴) 와 무관.
- S3 의 explorer mouse_move skip 은 explorer launch 600 ms 안에 mouse_move 이벤트 거의 없어서 효과 미반영.

#### Phase 1 → Phase 7 누적 비교

| 시나리오 | P1 (manual) | P5 (auto) | **P7 (auto)** | 메모 |
|---|---:|---:|---:|---|
| S1 | 2.20 G | 53.1 M | **37.1 M** | (수동/자동 분리) — auto baseline 대비 -30% |
| S2 | 48.03 G | 1,778.7 M | **1,825.0 M** | scrollwin / 텍스트 출력은 Phase 5 효과 후 추가 변경 없음 |
| S3 | 4.88 G | 57.8 M | **72.2 M** | explorer launch — Phase 5 baseline ±25% noise |
| S4 | 45.94 G | 82.9 M | **84.8 M** | Phase 5 baseline ±2% — tetris_t 가 boxfilwin × 200 → blit × 1 |
| S5 | 2.28 G | 57.1 M | **60.9 M** | sheet_slide 직접 호출 — 변경 없음 |

P1 (manual) → P5 (auto) 사이의 큰 절대값 차이는 **자동 측정에서 사용자 idle 시간이 빠진 것**. P5 → P7 은 같은 절차로 비교 가능.

#### Phase 7 결론

* **api_blit_rect 의 ABI 동작 검증 완료** ✓ — syscall 횟수 99% 감소, hrb_api/boxfill8 카운터로 명확히 보임.
* **단일 큰 refresh 가 다수 작은 refresh 보다 약간 빠르지만 큰 폭 아님**. dirty union 휴리스틱이 이미 충분히 효율적임을 역설적으로 검증.
* tetris_t 같은 "보드 = 한 사각형" 워크로드보다는, **disjoint 영역 다수 변경** 워크로드 (예: explorer 의 row 단위 update, settings 의 widget 단위 update) 가 batch syscall 의 진짜 타깃. 그러나 이런 워크로드는 현재 측정 시나리오에 없음 (S3/S4 모두 launch 직후 측정).
* 사용자 체감 응답성: settings 의 부분 redraw, explorer 의 hover-only mouse_move skip 은 **사람이 클릭/이동할 때의 응답성에 직접 기여** — 측정 시나리오로는 못 잡지만 실제 사용 경험 향상.
* **다음 (Phase 8) 의 진짜 목표**:
  - 회귀 smoke (work5 의 모든 동작 + 측정 시나리오 5개) — 변경 누적된 만큼 검증 필요.
  - 문서 (BXOS-COMMANDS.md / README.utf8.md / SETUP-MAC.md / he2/README.md) 갱신.
